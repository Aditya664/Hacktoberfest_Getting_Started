...
public:
  ...
  hash_table(int n) {
    table_hash_size = 8;
    ctr = 0;
    for (int i = 0; i < table_hash_size;
         i++) // increment hash table index as elements are inserted
    {
      table_hash[i] = -1;
    }
  }
  void
  store_value_linear_probin_with_replacemet(int value); // Function Declaration
...
int hash_table::search_value_in_hash_table_hash_linear_probing(
    int value) // Search Hash table with Linear Probing Function
{
  ...
  {
    cout << endl
         << "Given value " << value << " is stored at location " << hashvalue
         << " in hash table";
    return hashvalue;
  } else {
    ctr = 1;
    ...
    while (1) {
      hashvalue =
          (hashvalue + 1) % table_hash_size; // check for next hash location to
                                             // match with given hash value
      if (ctr == table_hash_size) // if matches with hash value
      {
        ...
        flag = 1;
        break;
      } else if (table_hash[hashvalue] ==
                 -1) // if element not present in hash table
      {
        flag = 2;
        break;
      }
    ...
    if (flag == 0) // if else statement
    {
      ...
      cout << "\nGiven value " << value
           << " is stored in hash table at location : " << hashvalue;
      return hashvalue;
    } else if (flag == 2) {
...
void hash_table::store_value_linear_probin_with_replacemet(
    int value) // Store Hash value with replacement Function
{
  int hashvalue = calculate_hash_value(value); // calcuate hash value
  // if the location is empty
  if (table_hash[hashvalue] == -1) // if else statement
  {
    table_hash[hashvalue] =
        value; // if value to inserted that hash value location is empty,then
               // insert it in hash location
    ctr++; // increment counter
  } else {
    int flag = 0;
    // from next location onward search linearly to find the empty location
    int temp_hash_value = calculate_hash_value(
        table_hash[hashvalue]);       // store that hash value in temp variable
    if (temp_hash_value == hashvalue) // if else statement
    {
      while (1) {
        hashvalue =
            (hashvalue + 1) %
            table_hash_size; // calcuate immediate next hash value location
    ...
    // else replace existing element in that hash location to next immediate
    // location and insert required value at that hash location
    {
...
void hash_table::display_hash_table_hash_linear_probing() // Display Hash table
                                                          // linear probing
                                                          // Function
{
