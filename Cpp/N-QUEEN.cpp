...
bool issafe2(int row, int col, vector<vector<char>> &pos, int n,
             vector<int> &rowhash, vector<int> &thirdhash,
             vector<int> &firsthash) {

  if (rowhash[row] == 1 || thirdhash[row + col] == 1 ||
...
bool findpos(int col, vector<vector<char>> &pos, int n, vector<int> &rowhash,
  ...
  for (int row = 0; row < n; row++) {
    ...
    if (issafe2(row, col, pos, n, rowhash, thirdhash, firsthash)) {
      ...
      thirdhash[row + col] = 1;
      firsthash[n - 1 - row + col] = 1;
      if (findpos(col + 1, pos, n, rowhash, thirdhash, firsthash) == true)
        return true;
      pos[row][col] = '.';
      rowhash[row] = 0;
      thirdhash[row + col] = 0;
